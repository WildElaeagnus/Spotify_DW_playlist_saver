...
def main():   # sourcery no-metrics
    ...
    PROGRAM_PATH = FILE_PATH.replace(__file__.split('\\')[-1], '')
    AUTH_INFO_PATH = str(str(PROGRAM_PATH) + "auth_info.txt")
    ...
    try:    
        ...
        if os.path.isfile(AUTH_INFO_PATH):
    ...
    def get_aval_device(*args, **kwargs):
        ...
        for i in devices:
            if (i['name'] == fav_device_name) or i['is_active'] == True: device_id = i['id']
        if device_id is None: device_id = devices[0]['id']
        return device_id
    # play detached song
